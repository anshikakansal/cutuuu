...
export const signUp = async (email, password, userType, profileData, locationData = {}) => {
  try {
    ...
    if (authError) {
      ...
      return handleError(authError, 'Auth signup');
    }

    if (!authData.user) {
      return handleError({ message: 'No user data returned from signup' }, 'Signup');
    }

    console.log('User created:', authData.user.id);

    // Create profile record - Fixed field mappings
    ...
    if (profileData.address) profilePayload.address = profileData.address;
    if (profileData.city) profilePayload.city = profileData.city;
    if (profileData.state) profilePayload.state = profileData.state;
    if (profileData.postalCode) profilePayload.postal_code = profileData.postalCode;
    ...
    console.log('Profile created successfully:', profileDataResponse);
...
export const signIn = async (email, password) => {
  try {
    ...
    try {
      ...
      }, 'Signed in successfully (profile fetch failed)');
    }
  } catch (error) {
    return handleError(error, 'Sign in');
  }
...
export const createMissingProfile = async (userId) => {
  try {
    ...
    if (existingProfile) {
      return handleSuccess(existingProfile, 'Profile already exists');
    }

    // Create new profile with proper default user_type
    const { data: newProfile, error: createError } = await supabase
      ...
      .insert({
        ...
        email: user.email,
        user_type: user.user_metadata?.user_type || 'citizen', // Fixed default
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
      })
      .select()
    ...
    if (createError) {
      return handleError(createError, 'Create missing profile');
    }

    return handleSuccess(newProfile, 'Profile created successfully');
  } catch (error) {
    return handleError(error, 'Create missing profile');
  }
};

export const checkProfileIntegrity = async () => {
  try {
    ...
    if (profileError) {
      return handleError(profileError, 'Profile integrity check');
    }

    if (!profile) {
      // Profile doesn't exist, create it
      console.log('Profile missing, creating...');
      return await createMissingProfile(user.id);
    }
    ...
    if (Object.keys(updates).length > 0) {
      ...
      return handleSuccess(updatedProfile, 'Profile updated');
    }

    return handleSuccess(profile, 'Profile is up to date');
  } catch (error) {
    return handleError(error, 'Profile integrity check');
  }
};
...
// Fixed resetPassword function for React Native
export const resetPassword = async (email) => {
  try {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      // Remove window.location.origin for React Native
      redirectTo: 'janconnect://reset-password', // Use deep linking for mobile
    });
...
export const getUserProfile = async (userId) => {
  try {
    ...
    // No profile found - return error instead of auto-creating
    console.log('No profile found for user:', userId);
    return handleError({ message: 'Profile not found' }, 'Get profile');
...
export const updateUserProfile = async (profileId, updates) => {
  try {
    if (!profileId) {
      return handleError({ message: 'Profile ID is required' }, 'Validation');
    }
    ...
    const { data, error } = await supabase
      ...
      .eq('id', profileId) // Fixed: Use profile ID, not user ID
      .select()
      .single();
...
// Location functions - Fixed to handle errors properly
export const getStates = async () => {
  try {
    ...
    if (error) {
      return handleError(error, 'Get states');
    }
...
export const getDistrictsByState = async (stateId) => {
  try {
    if (!stateId) {
      return handleError({ message: 'State ID is required' }, 'Validation');
    }
    ...
    if (error) {
      return handleError(error, 'Get districts');
    }
...
export const getAreasByDistrict = async (districtId) => {
  try {
    if (!districtId) {
      return handleError({ message: 'District ID is required' }, 'Validation');
    }
...
// Add verification email function that was missing
export const sendVerificationEmail = async (email) => {
  try {
    ...
    if (error) {
      return handleError(error, 'Send verification email');
    }

    return handleSuccess(null, 'Verification email sent');
  } catch (error) {
    return handleError(error, 'Send verification email');
  }
};
